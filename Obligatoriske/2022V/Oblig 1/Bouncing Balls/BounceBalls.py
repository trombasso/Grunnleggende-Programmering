...
class BounceBalls:
    def __init__(self):
        ...
        btFaster = Button(frame, text="Slower", command=self.slower)
        btFaster.pack(side=LEFT)
        """ ------------------------------------------------------------------------ """
    ...
    def faster(self):  # Increase speed
        self.sleepTime -= 5

    def slower(self):  # decrease speed
        self.sleepTime += 5
    ...
    def redisplayBall(self, ball):
        ...
        self.canvas.create_oval(
            ball.x - ball.radius, ball.y - ball.radius, ball.x + ball.radius, ball.y + ball.radius, fill=ball.color, tags="ball"
        )
